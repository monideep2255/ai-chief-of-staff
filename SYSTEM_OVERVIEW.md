...
This repo ships 33 rules. The full table, with exact scope and what each one enforces, lives in [CLAUDE.md](CLAUDE.md#rules). That table is the source of truth. Treat this section as the explanation of the mechanism, not a second copy of the list, so the two never drift apart.
