...
- Per-session tool-call cap: cap the total number of tool calls a single agent session or turn may make, separate from and in addition to the per-field `maxLength`/`maxItems` limits above. Those limits bound the size of any one payload; they do nothing to stop an agent from making an unbounded number of small, individually-valid calls, for example a retry loop that never terminates or a runaway recursive fan-out. A hard constant that stops the session once exceeded is the worked-example pattern. Set the cap per pipeline based on its expected call volume, and treat hitting it as a stop-and-report condition per `goal-contracts.md`, not a silent truncation.
- Provenance travels with every item, and its absence is the untrusted case. Each retrieved item carries a provenance field naming where it came from. The fetcher sets it at retrieval; no downstream stage may write or rewrite it, so a later stage cannot promote its own output into a source. An item arriving at any hop with provenance missing, empty, or unrecognized is handled as untrusted, never as ordinary. The intuitive default runs the other way, treating an unattributed item as unremarkable, and for a pipeline whose output carries citations that single default decides whether an unsourced claim can reach a cited answer. Pair it with the cite-or-refuse gate below: an item that cannot name its source cannot support a citation, so it must not be eligible to ground one.
